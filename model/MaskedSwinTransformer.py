...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=2,
                 ...
                 use_checkpoint=False, weight_init='', mask_ratio=0.0, **kwargs):
        super().__init__()
    ...
    def patchify(self, imgs, patchsize=4):
        p = patchsize
    ...
    def unpatchify(self, x, patchsize=4):
        p = patchsize
        h = w = int(x.shape[1] ** .5)
    ...
    def random_masking(self, x):
        ...
        if mask_ratio != 0.0:
            ...
            masked = torch.gather(x, dim=1, index=ids_keep.unsqueeze(-1).repeat(1, 1, 48))
            mas = nn.Parameter(torch.zeros(1, 1, 48))
            mask_tokens = mas.repeat(masked.shape[0], ids_restore.shape[1] - masked.shape[1], 1)
            mask_tokens = mask_tokens.to(torch.device('cuda:0'))
            x_ = torch.cat([masked[:, :, :], mask_tokens], dim=1)
            x_ = torch.gather(x_, dim=1, index=ids_restore.unsqueeze(-1).repeat(1, 1, masked.shape[2]))
